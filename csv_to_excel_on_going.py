...
def resolve_ongoing_input_files(input_path: Path) -> tuple[Path, Path] | None:
    ...
    tracking_files = sorted(
        (
            path
            for path in input_path.glob("Daily Tracking *.xlsx")
            if path.is_file() and not path.name.startswith("~$")
        ),
        key=lambda path: path.stat().st_mtime,
    )
    log_files = sorted(
        (path for path in input_path.glob("LogUpdateStatusOrderSD-*.csv") if path.is_file()),
        key=lambda path: path.stat().st_mtime,
    )

    if not tracking_files or not log_files:
        return None

    return tracking_files[-1], log_files[-1]


def ongoing_output_filename_from_tracking_workbook_path(tracking_workbook_path: Path) -> str:
...
def resolve_ongoing_output_path(input_path: Path, tracking_workbook_path: Path, args: argparse.Namespace) -> Path:
    default_filename = ongoing_output_filename_from_tracking_workbook_path(tracking_workbook_path)
    output = args.output
    if output is None:
...
def expected_on_progress_sheet_name(reference_date: date) -> str:
    return f"{ON_PROGRESS_SHEET_PREFIX} {reference_date.day} {reference_date.strftime('%B')}"[:EXCEL_MAX_SHEET_NAME_LENGTH]


def _normalize_pivot_item_name(value: object) -> str:
    return re.sub(r"\s+", " ", str(value).strip().lower())


def _set_pivot_field_visibility(field, visible_predicate) -> None:
    visible_items = []
    hidden_items = []
    for item in field.PivotItems():
...
def _apply_common_on_progress_filters(pivot_table) -> None:
    xl_page_field = 3
    ...
    try:
        ...
        year_field.Orientation = xl_page_field
        year_field.EnableMultiplePageItems = True
        _set_pivot_field_visibility(
            year_field,
            lambda item: not is_excluded_year_fab_upload_value(item.Value),
        )
    except Exception:
        pass

    try:
        process_adjustment_field = pivot_table.PivotFields(PROCESS_ADJUSTMENT_HEADER)
        process_adjustment_field.Orientation = xl_page_field
        process_adjustment_field.EnableMultiplePageItems = True
        _set_pivot_field_visibility(process_adjustment_field, lambda _item: True)
    except Exception:
        pass


def _apply_phase_exclusion_filter(pivot_table, excluded_values: set[str]) -> None:
    xl_page_field = 3
    excluded_normalized = {_normalize_pivot_item_name(value) for value in excluded_values}
    try:
        phase_field = pivot_table.PivotFields(PHASE_HEADER_PREFIX)
        phase_field.Orientation = xl_page_field
        phase_field.EnableMultiplePageItems = True
        _set_pivot_field_visibility(
            phase_field,
            lambda item: _normalize_pivot_item_name(item.Name) not in excluded_normalized,
        )
    except Exception:
        pass


def _apply_phase_prefix_filter(pivot_table, allowed_prefixes: tuple[str, ...]) -> None:
    xl_page_field = 3
    try:
        phase_field = pivot_table.PivotFields(PHASE_HEADER_PREFIX)
        phase_field.Orientation = xl_page_field
        ...
        _set_pivot_field_visibility(
            phase_field,
            lambda item: _normalize_pivot_item_name(item.Name).startswith(allowed_prefixes),
        )
    except Exception:
        pass


def update_on_progress_sheet_via_com(output_path: Path, reference_date: date) -> None:
    try:
        import win32com.client  # type: ignore
    except ImportError:
        raise RuntimeError("pywin32 is required to update the on progress sheet pivots.")
    ...
    try:
        ...
        if data_sheet is None or progress_sheet is None:
            raise RuntimeError("Could not find ALL ORDER and ON PROGRESS sheets.")

        target_progress_sheet_name = expected_on_progress_sheet_name(reference_date)
        if progress_sheet.Name != target_progress_sheet_name:
            progress_sheet.Name = target_progress_sheet_name
        try:
            progress_sheet.Tab.Color = int(EXCEL_ON_PROGRESS_TAB_COLOR, 16)
        except Exception:
            pass

        pivot_tables = progress_sheet.PivotTables()
        top_pivot_count = min(2, pivot_tables.Count)
        for pivot_index in range(1, top_pivot_count + 1):
            pt = pivot_tables(pivot_index)
            try:
                pt.ManualUpdate = True
            except Exception:
                pass
            _apply_common_on_progress_filters(pt)
            _apply_phase_exclusion_filter(pt, {"cancel", "so complete"})
            try:
                pt.ManualUpdate = False
                pt.RefreshTable()
            except Exception:
        ...
        try:
            third_pivot = progress_sheet.PivotTables(third_pivot_name)
        except Exception:
            third_pivot = None

        if third_pivot is None:
            ...
            third_pivot = shared_cache.CreatePivotTable(
                TableDestination=progress_sheet.Range(f"A{destination_row}"),
        ...
        try:
            row_field = third_pivot.PivotFields(STATUS_ORDER_HEADER)
            row_field.Orientation = xl_row_field
        ...
        try:
            count_field = third_pivot.AddDataField(third_pivot.PivotFields(QUO_HEADER), "Count of quo", xl_count)
            count_field.NumberFormat = "0"
        except Exception:
            pass
        ...
        _apply_common_on_progress_filters(third_pivot)
        _apply_phase_prefix_filter(third_pivot, ("04", "05", "06", "07"))
        ...
        try:
            third_top_row = third_pivot.TableRange2.Row
            progress_sheet.Range(f"A{third_top_row - 1}").Value = "Status Order Start From Pre-installation to UAT"
        except Exception:
            pass
...
def _match_column_name(columns: list[object], accepted_normalized_names: set[str]) -> str | None:
...
def load_pre_installation_start_date_lookup(log_csv_path: Path) -> dict[str, str]:
    ...
    quo_column = _match_column_name(lookup_df.columns.tolist(), {"quo", "#quo"})
    start_date_column = _match_column_name(
        lookup_df.columns.tolist(),
        {"startdate", "preinstallationstartdate"},
    )
    if quo_column is None or start_date_column is None:
        raise ValueError(
            f"Could not find required columns in {log_csv_path.name}. "
            "Expected columns for Quo (#Quo/Quo) and Start Date."
        )
...
def build_pre_installation_aging_ranges(aging_values: pd.Series) -> pd.Series:
    ranges = pd.Series(pd.NA, index=aging_values.index, dtype="string")
    valid = aging_values.notna()
    ranges.loc[valid] = "> 30 Days"
    ranges.loc[valid & (aging_values < 61)] = "15-30 Days"
    ranges.loc[valid & (aging_values < 31)] = "8-14 Days"
    ranges.loc[valid & (aging_values < 15)] = "1-7 Days"
    return ranges


def style_pre_installation_headers(worksheet, header_columns: list[int]) -> None:
...
def apply_pre_installation_columns(
    ...
    lookup = load_pre_installation_start_date_lookup(log_csv_path)
    reference_date = reference_date_from_tracking_workbook_path(tracking_workbook_path)

    wb = load_workbook(output_path)
    if VLOOKUP_SHEET_NAME not in wb.sheetnames:
        raise ValueError(f"Sheet '{VLOOKUP_SHEET_NAME}' not found in {tracking_workbook_path.name}.")

    ws = wb[VLOOKUP_SHEET_NAME]
    ...
    if anchor_column is None:
        raise ValueError(f"Header '{CATEGORY_MLD_HEADER}' not found in sheet '{VLOOKUP_SHEET_NAME}'.")

    quo_column = normalized_headers.get(normalize_header_key(QUO_HEADER))
    if quo_column is None:
        raise ValueError(f"Header '{QUO_HEADER}' not found in sheet '{VLOOKUP_SHEET_NAME}'.")
    ...
    existing_normalized_headers = set(normalized_headers.keys())
    for header in reversed(requested_headers):
        if normalize_header_key(header) not in existing_normalized_headers:
    ...
    style_pre_installation_headers(ws, [start_date_column, aging_column, range_column])

    quo_values = [
        normalize_quo_value(ws.cell(row=row_index, column=quo_column).value)
        for row_index in range(2, ws.max_row + 1)
    ]
    start_date_values = [lookup.get(quo_value, pd.NA) if quo_value else pd.NA for quo_value in quo_values]
    parsed_start_dates = pd.to_datetime(pd.Series(start_date_values, dtype="string"), errors="coerce", format="mixed")
    aging_values = pd.Series(pd.NA, index=parsed_start_dates.index, dtype="Int64")
    has_start_date = parsed_start_dates.notna()
    aging_values.loc[has_start_date] = (
        pd.Timestamp.now().normalize() - parsed_start_dates.loc[has_start_date].dt.normalize()
    ).dt.days
    range_values = build_pre_installation_aging_ranges(aging_values)

    for offset, row_index in enumerate(range(2, ws.max_row + 1)):
        start_date_value = start_date_values[offset]
        cell = ws.cell(row=row_index, column=start_date_column)
        if pd.isna(start_date_value) or not str(start_date_value).strip():
            cell.value = None
        else:
            parsed_value = pd.to_datetime(start_date_value, errors="coerce", format="mixed")
            if pd.isna(parsed_value):
                cell.value = str(start_date_value)
            else:
                cell.value = parsed_value.to_pydatetime()
                cell.number_format = "yyyy-mm-dd hh:mm:ss"

        ws.cell(
            row=row_index,
            column=aging_column,
            value=None if pd.isna(aging_values.iloc[offset]) else int(aging_values.iloc[offset]),
        )
        ws.cell(
            row=row_index,
            column=range_column,
            value=None if pd.isna(range_values.iloc[offset]) else str(range_values.iloc[offset]),
        )

    synchronize_worksheet_table_columns(ws)
    wb.save(output_path)
    update_on_progress_sheet_via_com(output_path, reference_date)


def synchronize_worksheet_table_columns(worksheet) -> None:
    """Keep Excel table XML columns in sync after structural column inserts."""
    for table in worksheet.tables.values():
        min_col, min_row, max_col, _ = range_boundaries(table.ref)
        header_values = [worksheet.cell(row=min_row, column=column_index).value for column_index in range(min_col, max_col + 1)]
...
def run_ongoing_two_file_mode(input_path: Path, args: argparse.Namespace) -> bool:
    input_files = resolve_ongoing_input_files(input_path)
    if input_files is None:
        return False

    tracking_workbook_path, log_csv_path = input_files
    output_path = resolve_ongoing_output_path(input_path, tracking_workbook_path, args).resolve()
    apply_pre_installation_columns(tracking_workbook_path, log_csv_path, output_path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if run_ongoing_two_file_mode(input_path, args):
            return 0
