...
const jobMonitor = window.createPersistentJobMonitor({
  storageKey: "document-csv-parser:report-2-active-job",
  statusPrefix: "/jobs/report-2/",
  button,
  statusBox,
  downloadLink,
  processingStage,
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const queued = await fetchJson("/convert/report-2/upload/jobs", {
      method: "POST",
      body,
    });
